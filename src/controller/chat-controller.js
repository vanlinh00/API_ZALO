...
let getconversation = async (req, res) => {
    ...
    var index = req.body.index;
    var count = req.body.count;
    if (token==undefined||partnerId == undefined||index==undefined||count==undefined) {
        Error.code1002(res);
    } 
    else if(token==""||partnerId==""||partnerId<=0||index==""||index<=0||count==""||count<=0)
    {
        Error.code1004(res);
    }
    else {
...
let deletemessage = async (req, res) => {
    ...
    else if(token == ""||partnerId == ""||partnerId <= 0||messageId==""||messageId<=0)
    {
        Error.code1004(res);
    }
    else {
...
let deleteConversation = async (req, res) => {
    ...
    else if(token == ""||partnerId == ""||partnerId <= 0)
    {
        Error.code1004(res);
    }
    else {
